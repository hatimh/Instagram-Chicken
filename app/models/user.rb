...
class User < ActiveRecord::Base
  ...
  def self.get_user_winning_pictures(client)
    winning_pictures = []
    all_winning_pictures = Picture.get_all_category_pictures_with_more_likes(client)
    all_winning_pictures.each do |picture|
      if picture.user_id == client.user.id
      winning_pictures << picture
      end
    end
    winning_pictures
  end
